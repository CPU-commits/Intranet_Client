...
export type GlobalForm = Omit<Omit<Omit<Omit<UserForm, 'items'>, 'author'>, 'upload_date'>, 'update_date'> & {
    items: Array<Omit<ItemType, 'questions'> & {
...
const useBuildFormStore = defineStore('build_form', {
    ...
    actions: {
        ...
        setInitQuestion() {
            if (this.editor && this.getCheckedItem.questions[this.question]?.question)
                this.editor.commands.setContent(
        ...
        deleteAnswer(index: number) {
            this.getCheckedItem.questions[this.getQuestion].answers.splice(index, 1)
        },
