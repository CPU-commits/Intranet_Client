...
export class LibraryService {
	...
	private validateBook(
		book: Book | Omit<Omit<Book & { tags: Array<string> }, '_id'>, 'image'>,
	) {
		if (book.name === '' || book.name.length > 150)
			throw new Error(
				'Debe existir un nombre de libro de máx. 150 cárac.',
			)
	...
	private buildDataBook(
		...
		if (filesImg && filesImg?.length > 0) form.append('image', filesImg[0])
		if (filesBook && filesBook?.length > 0)
			form.append('book', filesBook[0])
		return form
	}
	...
	async uploadBook(
		...
		try {
			...
			await this.nuxtApp.$fetchModule.fetchData({
				...
				body: this.buildDataUploadBook(book, filesImg, filesBook),
				spinnerStatus: true,
			})
			this.toastStore.addToast({
	...
	async updateEditorial(
		...
		try {
			if (editorial.length > 100)
				throw new Error('La editorial debe tener máx. 100 cárac.')
			const form = new FormData()
			form.append('editorial', editorial)
	...
	private validateAuthor(author: Omit<Omit<Author, '_id'>, 'date_update'>) {
		if (author.name === '' || author.name.length > 200)
			throw new Error(
				'Debe existir un nombre de autor de máx. 200 cárac.',
			)
		...
		author.table_info.forEach((item, i) => {
			if (item.key === '' || item.key.length > 50)
				throw new Error(
					`Debe existir un titulo para la tabla de información ${
						i + 1
					} de máx. 50 cárac.`,
				)
			if (item.value === '' || item.value.length > 100)
				throw new Error(
					`Debe existir un valor para la tabla de información ${
						i + 1
					} de máx. 100 cárac.`,
				)
		})
		...
		author.fun_facts.forEach((fact, i) => {
			if (fact.title === '' || fact.title.length > 100)
				throw new Error(
					`Debe existir un titulo para los datos curiosos ${
						i + 1
					} de máx. 100 cárac.`,
				)
			if (fact.fact === '' || fact.fact.length > 500)
				throw new Error(
					`Debe existir un dato curioso para los datos curiosos ${
						i + 1
					} de máx. 500 cárac.`,
				)
		})
	...
	private buildDataAuthor(
		author: Omit<Omit<Author, '_id'>, 'date_update'>,
		files: FileList | null,
	) {
		let form = new FormData()
