...
export class CourseService {
    ...
    async getStudentsFromCourse(idCourse: string) {
        const dataFetch = await this.nuxtApp.$fetchModule
            .fetchData<BodyFetch<Array<Student>> & DefaultResponse>({
                method: 'get',
    ...
    // Sections
    async newSection(section: string, files: FileList | null, idCourse: string) {
        try {
            if (section === '' || section.length > 100)
                throw new Error('Debe existir una sección con máx. 100 cárac.')
            if (!files || files?.length === 0) throw new Error('Debes subir una imágen')
            // Form
    ...
    async changeFile(sectionId: string, files: FileList | null) {
        try {
            ...
            form.append('image', files[0])
            const dataFetch = await this.nuxtApp.$fetchModule
                .fetchData<BodyFetch<string> & DefaultResponse>({
                    method: 'put',
