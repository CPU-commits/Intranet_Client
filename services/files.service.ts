...
export class FilesService {
    ...
    async getFiles() {
        const dataFetch = await this.nuxtApp.$fetchModule
            .fetchData<BodyFetch<Array<UserFile>> & DefaultResponse>({
                method: 'get',
    ...
    private validatorsFile(files: FileList | null, fileForm: { title: string }) {
    ...
    async uploadFile(files: FileList | null, fileForm: { title: string }) {
        try {
            ...
            // Send request
            const dataFetch = await this.nuxtApp.$fetchModule
                .fetchData<BodyFetch<UserFile> & DefaultResponse>({
                    method: 'post',
