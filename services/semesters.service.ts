...
export class SemestersService {
    ...
    async getSemesterYear(year: number) {
        ...
        const finishedSemesters = dataFetch.body.semesters.filter((semester) => {
            if (semester.status === 0) return semester
        })

        const promises = {
			first: {
				promises: [] as Array<Promise<{
                    body: {
