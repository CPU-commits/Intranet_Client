...
export class TeacherService {
    ...
    async getTeachers(total = false, skip?: number, search?: string) {
        ...
        const dataFetch = await this.nuxtApp.$fetchModule
            .fetchData<BodyFetch<Teachers> & DefaultResponse>({
                URL,
                method: 'get',
                spinnerStatus: true,
    ...
    private validatorsTeacher(formTeacher: Omit<Omit<User, '_id'>, 'user_type'>) {
    ...
    async uploadTeachers(teachers: Array<Omit<Omit<User, '_id'>, 'user_type'>>) {
        try {
    ...
    async addSubjectCourse(subject: string, idTeacher: string) {
        try {
            ...
            const dataFetch = await this.nuxtApp.$fetchModule
                .fetchData<BodyFetch<Teacher> & DefaultResponse>({
                    method: 'post',
    ...
    async deleteSubjectCourse(idImparted: string, idTeacher: string) {
        try {
            ...
            this.toastsStore.addToast({
                message: 'Se ha eliminado la materia - curso al profesor exitosamente',
                type: 'success',
            })
            return true
        } catch (err) {
