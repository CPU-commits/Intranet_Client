...
// Nuxt app
const { $fetchModule, $semesterService, $booklifeService } = useNuxtApp()
// Stores
...
<template>
	<section class="Booklife">
		<aside class="Semesters">
			<ul>
				<li v-for="(semester, i) in semesters" :key="i">
		...
		<section v-if="semester" class="Observations">
			<h3>
			...
			>
				...
				<HTMLTextArea
					...
					:keydown="
						() =>
							$booklifeService.updateObservation(
		...
		<Modal v-model:opened="modalAdd">
			...
			<HTMLForm :form="uploadObservation">
				...
				<HTMLSelect id="type" v-model:value="observationForm.type">
