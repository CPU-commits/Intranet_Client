...
async function likeNews() {
    toogleLike()
    const liked = await $newsService.likeNews(
        news.like,
        news._id,
    )
    if (!liked) toogleLike()
...
async function deleteNews() {
    modal.value = false
    const deleted = await $newsService.deleteNews(news._id)
...
<template>
    <article bind:this={newsElement} class="News">
        ...
        >
            ...
            <div :class="openMenu ? 'News__config--open' : ''" class="News__config--container">
        ...
        <footer class="News__details">
            <button @click="likeNews"> 
                <i :class="news.like ? 'LikeAnimation' : ''" class="fa-solid fa-heart" />
            </button>
