...
<template>
	<header class="Header">
		...
		<section class="Header__subject">
			...
			<section v-if="panel.isPanel" class="Header__subject--sub_sections">
				<NuxtLink
					...
					class="SubSection"
					:class="
						i === parseInt(panel.section) ? 'ButtonSelected' : ''
					"
				>
					{{ subSection.name }}
				</NuxtLink>
				<span
					v-if="
						!_module.sub_sections ||
						_module.sub_sections.length === 0
					"
				>
					Sin sub-secciones...
				</span>
		...
		<Modal v-model:opened="modal">
			...
			<HTMLForm :form="addSection">
				...
				<HTMLInput id="sub-section" v-model:value="subSection" />
