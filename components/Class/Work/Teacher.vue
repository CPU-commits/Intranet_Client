...
async function downloadFiles(idStudent: string, index: number) {
    const urlToken = await $workService.downloadFiles(
        idStudent,
        work._id,
    )
    if (urlToken) {
...
<template>
    <section>
                ...
                <template v-if="dateIsBefore(work.date_limit, now)">
                    <template v-if="work.type === 'form'">
                        <template v-if="formHasPoints">
                            <td>
                                <i
                                    v-if="student.access && student.evaluate?.percentage !== 100"
                            ...
                            <td>
                                <HTMLAIcon
                                    v-if="student.access && student.access.status !== 'opened'"
                    ...
                    <template
                        v-else-if="work.type === 'files' && dateIsBefore(work.date_limit, now)"
                    >
                        <td v-if="student.files_uploaded">
                            <HTMLButton
                                :click="() => downloadFiles(student.user._id, i)"
                                type="button"
                            >
                        ...
                        <td
                            :class="dateIsBefore(student?.files_uploaded?.date, work.date_limit)
                                ? 'Success'
                                : 'Fail'"
                        >
                            ...
                            {{ dateIsBefore(student?.files_uploaded?.date, work.date_limit)
                                ? '(A tiempo)'
                                : '(Atrasado)' }}
                        </td>
                        <td>
        ...
        <span class="Revised" v-if="dateIsBefore(work.date_limit, now) && work.is_revised">
            <i class="fa-solid fa-circle-check" /> Trabajo revisado
        </span>
        <footer v-else-if="dateIsBefore(work.date_limit, now) && (formHasPoints || work.type === 'files')" class="Button">
            <br />
            <HTMLButton
                :click="() => {
                    if (work.type === 'files') {
        ...
        <Modal v-model:opened="modal">
            ...
            <HTMLForm :form="uploadEvaluateFiles">
                <HTMLTable :header="['Titulo', 'Descr.', 'Pts. Máx', 'Pts. Evaluados']">
