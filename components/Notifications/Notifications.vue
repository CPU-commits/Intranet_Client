...
async function getNotifications(total = false, skip = 0) {
    try {
        const dataFetch = await $notificationService.getNotifications(total, skip)
...
<template>
    <section class="Container">
        <button @click="toogleMenu">
            ...
            />
            <span v-if="notificationsNumber && notificationsNumber < 100" class="notifications">
                {{ notificationsNumber }}
            </span>
        ...
        <section v-if="notificationsOpen" ref="notificationEl" class="Notifications">
