...
import type {
	ClassroomModule,
	DirectiveModule,
} from '~~/models/classroom/modules.model'
...
<template>
	<NuxtLayout name="admin">
		...
		<AdminPanel>
			...
			<!-- Data -->
			<HTMLTable
				:header="['Módulo', 'Estado', 'Calificaciones', 'Confg.']"
			>
		...
		<Modal v-model:opened="modalGrade">
			...
			<HTMLForm :form="() => $gradesService.updateGrades(gradeConfig)">
				...
				<HTMLButton type="submit"
					>Actualizar configuraci&oacute;n</HTMLButton
				>
			</HTMLForm>
		</Modal>
		...
		<Modal v-model:opened="modalGrades">
			<template #title>
				<h2>
					...
					{{ moduleSelected.section.section }} ° -
					{{ moduleSelected.subject.subject }}
				</h2>
			</template>
		...
		<Modal v-model:opened="modalConfg">
			<template #title>
				<h2>Configuraci&oacute;n de m&oacute;dulo</h2>
			</template>
			<HTMLForm :form="() => addDirective(moduleSelected._id)">
				<h3><i class="fa-solid fa-sign-hanging" /> Directivas</h3>
				<label for="min"
					>Cantidad m&iacute;nima de calificaciones</label
				>
				<HTMLCheckbox
					id="min_actived"
					v-model:checked="directives.min_grades.actived"
				/>
				<HTMLInput
				...
				<label for="continuous"
					>Calificaciones programadas continuas entre s&iacute;</label
				>
				<HTMLCheckbox
					id="continuous"
					v-model:checked="directives.continuous"
				/>
				...
				<HTMLCheckbox
					id="all_grades"
					v-model:checked="directives.all_grades"
				/>
		...
		<Modal v-model:opened="modalDirectives">
			<template #title>
				<h2>Directivas de m&oacute;dulos</h2>
			</template>
			<HTMLTable
				:header="[
					'Módulo',
					'Estado directivas',
					'Directivas por completar',
				]"
			>
