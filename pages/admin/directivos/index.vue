...
const title = schoolName
	? `Directivos - Admin - ${schoolName} - Intranet`
	: 'Directivos - Admin - Intranet'
// Guard
definePageMeta({
	middleware: 'role',
...
// Nuxt app
const { $fetchModule, $directivesService } = useNuxtApp()
...
async function uploadDirective() {
	const dataFetch = await $directivesService.uploadDirective(
		formDirective as User,
	)
	...
	<NuxtLayout name="admin">
		...
		<AdminPanel>
			...
			<HTMLSearch
				...
				:search="() => getDirectives(true, 0, search)"
			/>
			<br />
			<HTMLTable
				v-if="directives"
			...
			>
				<tr
					v-for="(directive, i) in directives.users"
					:key="directive._id"
				>
		...
		<Modal v-model:opened="modal">
			...
			<HTMLForm :form="uploadDirective">
				...
				<HTMLInput id="name" v-model:value="formDirective.name" />
				<label for="fln">Apellido Paterno</label>
				<HTMLInput
					id="fln"
					v-model:value="formDirective.first_lastname"
				/>
		...
		<Modal v-model:opened="modalEdit">
			...
			<HTMLForm v-if="directiveEdit" :form="editDirective">
				...
				<HTMLInput id="nameE" v-model:value="directiveEdit.name" />
				<label for="flnE">Apellido Paterno</label>
				<HTMLInput
					id="flnE"
					v-model:value="directiveEdit.first_lastname"
				/>
		...
		<Modal v-model:opened="modalStatus" :fn="() => (modalEdit = true)">
			<template #title>
				<h2>
					...
					{{
						directives?.users[directivePosition].status === 1
							? 'Dar de baja'
							: 'Reintegrar'
					}}
				</h2>
