...
<template>
	<NuxtLayout name="admin">
		...
		<AdminPanel :nav="false">
			<h1>
				<i class="fa-solid fa-folder-open" /> {{ year }} - Archivado
			</h1>

			<section v-for="(semester, i) in semesters" :key="i" class="Folder">
				<h3>{{ semesterTitle(semester.semester) }}</h3>
				<section v-if="semester.semester.status === 0">
					<h4>
						<i class="fa-solid fa-box" /> M&oacute;dulos - Aula
						Virtual
					</h4>
					<br />
					<HTMLTable
						v-if="semester.modules"
						:header="['Módulo', 'Calificaciones', 'Exportar']"
					>
						<tr v-for="(_module, i) in semester.modules" :key="i">
							...
							<td class="Button">
								<HTMLButton
									:click="() => exportGrades(_module._id)"
									type="button"
								>
					...
					<section v-if="semester.semester.semester === 2">
						<h4>
							<i class="fa-solid fa-repeat" /> Estudiantes
							repitentes
						</h4>
						<HTMLTable
							v-if="semester?.students?.students"
							:header="['Nombre', 'RUT']"
						>
							<tr
								v-for="(student, i) in semester.students
									.students"
								:key="i"
							>
								<td>
						...
						<p v-if="semester?.students?.students?.length === 0">
							Ning&uacute;n estudiante repitente
						</p>
						<br />
						<small>
							Repitentes totales:
							{{ semester.students?.students?.length ?? 0 }}
						</small>
					</section>
				</section>
			</section>
