			...
			<HTMLForm v-if="author" :form="updateAuthor">
				<section class="Header">
					<article class="Header__essentials">
						...
						<HTMLInput id="name" v-model:value="author.name" />
						<label for="biography">Biograf&iacute;a</label>
						<HTMLTextArea
							id="biography"
							v-model:value="author.biography"
						/>
					</article>
					<figure class="Header__image">
						...
						<span v-if="author.table_info.length === 0"
							>Sin elementos...</span
						>
				...
				<section class="Fun_facts">
					...
					<span v-if="author.fun_facts.length === 0"
						>Sin datos curiosos...</span
					>
				</section>
				<HTMLButtonIcon
				...
				/>
				<h3>
					<i class="fa-solid fa-book-atlas" /> Referencias
					(Preferentemente APA) *
				</h3>
				<article
					v-for="(_, i) in author.references"
					:key="i"
					class="Reference"
				>
					<HTMLInput
						v-model:value="author.references[i]"
